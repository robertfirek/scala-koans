...
class AboutTraversables extends KoanSuite with ShouldMatchers {
  ...
  koan( """Traversables are the superclass of Lists, Arrays, Maps, Sets, Streams, and more.
...
//
//    }
    val result = list.collect(partialFunction1)
  ...
  koan( """Drop will take the rest of the Traversable except
    ...
    def streamer(v: Int): Stream[Int] = cons(v, streamer(v + 1))
    val a = streamer(2)
    a.slice(6, 9).toList should be(List(8, 9, 10))
  }
